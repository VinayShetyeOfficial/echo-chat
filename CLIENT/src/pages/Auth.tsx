...
export default function Auth() {
  ...
  const validatePassword = (value: string): boolean => {
    ...
    if (!(hasUppercase && hasLowercase && (hasNumber || hasSpecial))) {
      setPasswordError("Password must include uppercase, lowercase, and a number or special character")
      return false
    }
  ...
  const getCardHeader = () => (
    <CardHeader className="space-y-2 p-6 pb-4 bg-gray-950/30">
      ...
      <CardDescription className="text-center text-gray-300">
        {inviteData ? (
          <>
            <span className="font-medium">{inviteData.invitation.createdBy.username}</span> has invited you to join{" "}
        ...
        <CardContent className="p-6 pt-5 pb-8 bg-gray-950/20">
          ...
          >
            <div className="auth-tabs-list">
              <div
                className={`auth-tab ${activeTab === "login" ? "auth-tab-active" : "auth-tab-inactive"}`}
                onClick={() => {
              ...
              <div
                className={`auth-tab ${activeTab === "signup" ? "auth-tab-active" : "auth-tab-inactive"}`}
                onClick={() => {
                  // Clear all errors when clicking tab directly
                  setError("")
                  setSuccess("")
            ...
            <TabsContent value="login" className="">
              <div className="bg-gray-900/70 p-4 rounded-lg">
                <form onSubmit={handleLoginSubmit} className="space-y-4">
                  {success && <div className="p-2 mb-4 bg-green-100 text-green-700 rounded">{success}</div>}

                  {error ? (
                    <div className="p-3 bg-red-500/20 border border-red-500 rounded-md text-red-200 text-center mb-4">
                      <p className="font-medium">
                        {error.toLowerCase().includes("wrong") ? error : "Wrong email or password"}
                      </p>
                    </div>
                  ) : null}
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="password" className="text-left block text-sm font-medium text-gray-200">
                      Password
                    </Label>
                    ...
                    {!passwordError && (loginPasswordFocused || loginForm.password !== "") && (
                      <p className="text-gray-500 text-xs mt-1">
                        Password must be at least 8 characters and include uppercase, lowercase, and a number or special
                        character.
                      </p>
                    )}
                  </div>
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor="remember-me" className="text-sm font-medium text-gray-300 cursor-pointer">
                      Remember me
                    </Label>
                  </div>
                  <Button
            ...
            <TabsContent value="signup" className="pb-6">
              <div className="bg-gray-900/70 p-4 rounded-lg">
                <form onSubmit={handleSignupSubmit} className="space-y-4">
                  ...
                  {Object.values(serverErrors).some(Boolean) && !error && (
                    <div className="p-3 bg-red-500/20 border border-red-500 rounded-md text-red-200 text-sm mb-4">
                      ...
                      <ul className="list-disc pl-5 mt-1">
                        {serverErrors.username && <li>{serverErrors.username}</li>}
                        {serverErrors.email && <li>{serverErrors.email}</li>}
                        {serverErrors.password && <li>{serverErrors.password}</li>}
                      </ul>
                    </div>
                  )}

                  <div className="space-y-2">
                    <Label htmlFor="username" className="text-left block text-sm font-medium text-gray-200">
                      Username
                    </Label>
                    <Input
                      id="username"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="signup-email" className="text-left block text-sm font-medium text-gray-200">
                      Email
                    </Label>
                    <Input
                      id="signup-email"
                      ...
                      placeholder="Enter your email address"
                      className={`bg-gray-800/50 border-gray-700 text-white placeholder:text-gray-500 ${
                        emailError || serverErrors.email || (error && error.includes("email already exists"))
                          ? "border-red-500"
                          : ""
                      }`}
                    />
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="signup-password" className="text-left block text-sm font-medium text-gray-200">
                      Password
                    </Label>
                    <div className="relative">
                      <Input
                  ...
                  <Button type="submit" className="w-full mt-4 bg-primary hover:bg-primary/90 text-white py-2">
