...
export function ChatSidebar({ isOpen, onToggle, onCreateChannel }: ChatSidebarProps) {
  ...
  const processedChannels = channels.reduce((acc: { channel: Channel; recipient?: User }[], channel) => {
    ...
    if (channel.type === "direct" && user) {
      ...
      for (const member of members) {
        ...
        if (member.userId && member.userId !== user.id) {
          if (member.user && member.user.username) {
            ...
            const userId = typeof member.userId === "object" ? JSON.stringify(member.userId) : member.userId
            console.log(`Direct message: User details needed for ID: ${userId}`)
          }
        }
        // Case 2: Member is a User object
      ...
      if (recipient && recipient.username) {
        console.log(`Direct message: Found recipient ${recipient.username} for channel ${channel.id || "new"}`)
  ...
  const sortedChannels = [...filteredChannels].sort((a, b) => {
    const dateA = new Date(a.channel.lastMessage?.timestamp || a.channel.createdAt || 0)
    const dateB = new Date(b.channel.lastMessage?.timestamp || b.channel.createdAt || 0)
  ...
  return (
    <>
      {isOpen && <div className="fixed inset-0 bg-black/30 z-40 md:hidden" onClick={onToggle} />}
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700">
          <button
            ...
            className={cn(
              ...
              activeTab === "all" ? "text-chat-primary" : "text-muted-foreground hover:text-foreground",
            )}
          >
            All
            {activeTab === "all" && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-chat-primary" />}
          </button>
          <button
            onClick={() => setActiveTab("direct")}
            className={cn(
              "flex-1 py-2.5 text-sm font-medium transition-colors relative",
              activeTab === "direct" ? "text-chat-primary" : "text-muted-foreground hover:text-foreground",
            )}
          >
            Direct
            {activeTab === "direct" && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-chat-primary" />}
          </button>
          <button
            onClick={() => setActiveTab("groups")}
            className={cn(
              "flex-1 py-2.5 text-sm font-medium transition-colors relative",
              activeTab === "groups" ? "text-chat-primary" : "text-muted-foreground hover:text-foreground",
            )}
          >
            Groups
            {activeTab === "groups" && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-chat-primary" />}
          </button>
        </div>
          ...
          {visibleChannels.length === 0 ? (
            <div className="px-4 py-8 text-center flex flex-col items-center">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                {searchTerm ? "No conversations match your search." : "No channels or messages yet."}
              </p>
        ...
        <div className="mt-auto border-t p-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate text-left">{user?.username || "Unknown User"}</p>
              <p className="text-xs text-muted-foreground truncate text-left">{user?.email || ""}</p>
            </div>
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
