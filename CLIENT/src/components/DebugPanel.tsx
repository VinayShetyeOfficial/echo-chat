  ...
  const log = {
    success: (message: string, data?: any) => {
      ...
      if (!prevLogsRef.current[key] || now - prevLogsRef.current[key] > 1000) {
        ...
        setLogs((prev) => [{ type: "success", message, data, timestamp: new Date() }, ...prev.slice(0, 99)])
      }
    },
    error: (message: string, data?: any) => {
      setLogs((prev) => [{ type: "error", message, data, timestamp: new Date() }, ...prev])
    },
    info: (message: string, data?: any) => {
      setLogs((prev) => [{ type: "info", message, data, timestamp: new Date() }, ...prev])
    },
    clear: () => setLogs([]),
  }
...
export const DebugContext = createContext<ReturnType<typeof useDebugLogger> | undefined>(undefined)
...
export const DebugPanel = () => {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      <button onClick={() => setIsOpen(!isOpen)} className="bg-purple-600 text-white px-3 py-1 rounded-md">
            ...
            {logs.map((log, i) => (
              <div
                ...
                className={`p-2 rounded text-sm ${
                  log.type === "success" ? "bg-green-900/50" : log.type === "error" ? "bg-red-900/50" : "bg-blue-900/50"
                }`}
              >
                ...
                {log.data && <pre className="text-xs mt-1 overflow-x-auto">{JSON.stringify(log.data, null, 2)}</pre>}
              </div>
            ))}
          </div>
        </div>
